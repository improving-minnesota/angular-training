...
- What have we created here?
  - A `requestEmployees` function and added it to the controller's scope.
    ...
    - The result of the list is then assigned to the `employees` variable on the controller's scope.
  - A `remove` function on the controller that takes an `employee` object as a param.
  ...
  - A `restore` function on the controller that in turn calls the `restore()` function of the `data` service.
...
- The final line of code calls `vm.requestEmployees(1)` to initialize the list of employees when the controller is initialized.
...
- Locate the `TODO` near line #50 and tell the `$httpBackend` service how to respond to a request for a list of users:
...
###### Test that employees is populated on controller scope

- Locate the `TODO` near line #67 and replace it a test that :
  - Sets an expectation on the `$httpBackend` service for a GET request to ***/users***.
  - Calls `controller.requestEmployees()`
  - Flushes the `$httpBackend` service
  - Tests that the result of the requesting employees is set on the controller.
...
- Locate the `TODO` near line #79 and replace it with a test that :
  ...
  - Calls `controller.remove(employee)`
...
- Locate the `TODO` near line #91 and set the response to:

```javascript
$httpBackend.when('PUT', '/users/' + employee._id).respond(200);
```

###### Test the behavior of a calling `controller.remove()`

- Near line #95, replace the `TODO` with the following test:
  - Call the remove function on controller.
...
- Locate the `TODO` near line #107 and set the response:
...
- Find the `TODO` near line #112 and replace it with a test that:
...
- Locate the `TODO` near line #127 and replace it with a test that:
  ...
  - Calls the `restore()` function on controller scope and passes in the `employee`.
...
- Locate the `TODO` near line #140 and configure a successful response:

```javascript
$httpBackend.when('PUT', '/users/' + employee._id).respond(200);
```
- Now locate the `TODO` near line #143 and test that the employee is restored:
  - Call the `restore` function on controller scope
...
- Locate the `TODO` near line #155 and set an error response for a restore request:

```javascript
$httpBackend.when('PUT', '/users/' + employee._id).respond(500);
```

- Next, find the `TODO` near line #158 and test the error:
  - Call the `restore()` function on controller scope.
  - Flush the http backend.
...
- Let's first set up a `ng-repeat` directive to loop through our `employeeCtrl.employees` array that gets set up in our `requestEmployees()` controller function.
...
```xml
<tr ng-repeat="timesheet in timesheetCtrl.timesheets"
  class="fadeable-row"
...
<div ng-switch="timesheet.deleted">
  <div ng-switch-when="true">
    <button class="btn btn-sm btn-default" ng-click="timesheetCtrl.restore(timesheet); $event.stopPropagation();">Restore</button>
  </div>
  <div ng-switch-default>
      <button class="btn btn-sm btn-danger" ng-click="timesheetCtrl.remove(timesheet); $event.stopPropagation();">Delete</button>
  </div>
</div>
```
